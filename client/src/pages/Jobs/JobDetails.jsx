      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-6">
            ...
            <h2 className="text-2xl text-gray-200">{job?.company?.companyName}</h2>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start mb-8 space-y-6 md:space-y-0">
          ...
          <Button className="w-full md:w-auto px-8 py-4 text-lg font-semibold">Apply Now</Button>
        </div>
        ...
        <section className="mb-12">
          ...
          <div className="flex flex-wrap gap-3">
            {job?.requirements?.map((req, index) => (
              <Badge key={index} variant="secondary" className="text-sm py-2 px-4">{req}</Badge>
            ))}
          </div>
        </section>
